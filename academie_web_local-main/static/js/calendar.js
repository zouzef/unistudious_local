document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!calendarInfoEl) {
    console.log(
      "Calendar info element not found - skipping calendar initialization",
    );
    ...
    drop: function (info) {
      ...
      modalElement.addEventListener(
        ...
        function () {
          ...
          const eventGroupCapacity = document.getElementById(
            "createEventGroupCapacity",
          );
          const eventSessionId = document.getElementById(
            "createEventSessionId",
          );
          const eventAccountId = document.getElementById(
            "createEventAccountId",
          );

          if (eventTitle) eventTitle.value = groupName;
          if (eventDate)
            eventDate.value = droppedDate.toISOString().split("T")[0];
