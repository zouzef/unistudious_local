...
def get_enrolled_subjects() -> set:
    """
    Fetch all subjects already enrolled in CompreFace.
    Returns a set of subject names (user IDs as strings).
    """
    try:
        response = requests.get(
            SUBJECTS_ENDPOINT,  # e.g. "http://localhost:8000/api/v1/recognition/faces/subjects"
            headers=HEADERS,
            timeout=10
        )
...
def enroll_students(students: list) -> int:
    """
    Enroll students reference photos into CompreFace Recognition service.
    Subject name = userId.
    Up to MAX_ENROLL_IMAGES images per student.
    Skips students already enrolled in CompreFace.

    Returns number of successfully enrolled students.
    """
    enrolled = 0

    # Fetch already-enrolled subjects once before the loop
    ...
    for student in students:
        ...
        # ✅ Skip if already enrolled in CompreFace
...
def unenroll_students(students: list) -> None:
    """
    Remove all enrolled faces for each student from CompreFace after session ends.
    """
    for student in students:
        user_id = student.get("userId")
        if not user_id:
...
def recognize_persons(calendar_id: int) -> dict:
    """
    For each person_X folder in classified output, send ALL images to CompreFace
    and get back the most frequent matched userId (subject).
    Rejects matches below RECOGNITION_THRESHOLD.
    Prevents the same userId from being assigned to multiple persons.

    Returns a dict: { user_id: avg_similarity } for recognized persons.
    """
    ...
    for person in person_folders:
        ...
        votes = {}  # { user_id: [similarity1, similarity2, ...] }
        ...
        # Pick userId with most votes, break ties by highest avg similarity
        ...
        # ✅ Reject if userId already assigned to another person
