...
from services.attendance_service import update_attendance, get_present_students
...
from detection.recognition import enroll_students, unenroll_students
        ...
        try:
            ...
            if not calendars:
                ...
                for calendar in calendars_soon:
                    ...
                    # ✅ START CAMERA DETECTION + RECOGNITION WORKER
                    recognition_queue = Queue(maxsize=50)  # ← create queue here
                    processes, stop_event = start_detection_for_calendar(
                        ...
                        client=client,
                        recognition_queue=recognition_queue  # ← pass it
                    )
                    ...
                    active_calendars.append({
                        ...
                        "recognition_queue": recognition_queue,  # ← store it
                    })
                    logger.info(f"Calendar {calendar['id']} started — {len(processes)} process(es) running.")
            ...
            for active in active_calendars:
                if now >= active["end_time"]:
                    ...
                    # ✅ STOP ALL PROCESSES (cameras + recognition worker)
                    ...
                    # ✅ CLASSIFY REMAINING FACE CROPS (unknown persons only)
                    classify_faces(active["calendar_id"])

                    # ✅ STORE CLASSIFIED FOLDERS IN DB
                    store_classified_folders(client, active["calendar_id"])

                    # ✅ OPTION B: Query DB to get already-present students
                    # recognition worker already marked some students present in real-time
                    already_present = get_present_students(client, active["calendar_id"])
                    logger.info(f"Already present students (real-time): {already_present}")

                    # ✅ UPDATE ATTENDANCE ONLY FOR ABSENT STUDENTS
                    # don't overwrite students already marked present by recognition worker
                    attendance_map = {
                    ...
                    for student in active["attendances"]:
                        ...
                        # ✅ Skip if already marked present by recognition worker
                        if user_id in already_present:
                            logger.info(f"Student {user_id} already present — skipping.")
                            continue

                        # ✅ Mark as absent
                        update_attendance(client, attendance_id, False)
                        logger.info(f"Student {user_id} marked absent.")

                    # ✅ UNENROLL REMAINING STUDENTS FROM COMPREFACE
                    # (recognized students already unenrolled by recognition worker)
                    logger.info(f"Unenrolling remaining students for calendar {active['calendar_id']}...")
