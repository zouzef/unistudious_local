...
# AUDIT HELPER
# ========================================
def log_session_audit(action_type, old_data, new_data, id_session, is_synced=0):
	"""
    Insert an audit record into the session_audit table.

    Args:
        action_type: 'INSERT' | 'UPDATE' | 'DELETE'
        old_data: dict (or None) - state before the change
        new_data: dict (or None) - state after the change
        id_session: the session id this audit row refers to
        is_synced: 0 = not yet pushed to production (default), 1 = synced
    """
	try:
		audit_query = """
		...
		# Don't let an audit failure break the main operation, just log it
		print(f"⚠️  Failed to write session_audit ({action_type}, id_session={id_session}): {e}")
...
def delete_session(session_id):
	try:
		# First check if session exists and is enabled (also use it as old_data for audit)
		check_query = "SELECT * FROM session WHERE id = %s AND enabled = 1"
		old_records = Database.execute_query(check_query, (session_id,), fetch=True)
		...
		# Audit record — new_data is NULL since the record is being (soft) deleted
		log_session_audit(
			action_type='DELETE',
			old_data=old_data,
			new_data=None,
