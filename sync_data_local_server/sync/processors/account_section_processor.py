"""
Account Section Data Processor
Handles inserting and updating account_section records in the database
"""
import sys
import os
...
def insert_account_section(db, section_data):
    """
    ...
    try:
        ...
        if not created_records:
            print("   ℹ️  No account_section records in 'created'")
            return result

        print(f"   Processing {len(created_records)} account_section record(s) from 'created'...")
        ...
        for i, record in enumerate(created_records, 1):
            try:
                ...
                print(f"   [{i}/{len(created_records)}] Account Section ID {record_id}...")
                ...
                print(f"      ❌ Error processing account_section ID {record.get('id', 'unknown')}: {err}")
                result["errors"] += 1
                continue
        ...
        print(f"   💥 Unexpected error in insert_account_section: {err}")

    return result


def update_account_section(db, section_data):
    """
    Handle 'updated' account_section records from API
    Logic:
    - If record exists in DB → UPDATE it
    ...
    try:
        ...
        if not updated_records:
            print("   ℹ️  No account_section records in 'updated'")
            return result

        print(f"   Processing {len(updated_records)} account_section record(s) from 'updated'...")
        ...
        print(f"   💥 Unexpected error in update_account_section: {err}")

    return result


def processor_account_section(db, section_data):
    """
    Process account_section data (handles both 'created' and 'updated' sections)
    ...
    if section_data.get("created"):
        ...
        results["created_section"] = insert_account_section(db, section_data)

    # Process 'updated' section
    if section_data.get("updated"):
        print(f"\n🔄 Processing 'updated' section ({len(section_data['updated'])} records)...")
        results["updated_section"] = update_account_section(db, section_data)
    ...
    print("\n" + "=" * 60)
    print("📊 ACCOUNT SECTIONS - TOTAL SUMMARY")
    print("=" * 60)
