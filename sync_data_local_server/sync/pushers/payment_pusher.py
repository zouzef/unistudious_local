...
def _send_Update_Payment_Not_Register_Api(settings, payload):
	try:
		token = get_token()
		headers = {"Authorization": f"Bearer {token}"}
		url = f"{settings.api_base_url}/slc/update-payment-status-not-registered"
		...
		print(f"Error: {e} in _send_Update_Payment_Not_Register_Api")
		return False
...
def push_Payment_Save(db, settings, row):
	try:
		...
		if not UserIdProd or not SessionIdProd or not PaymentIdProd:
			logger.error("Missing id_prod for user_id=%s, session_id=%s, payment_id=%s", user_id, session_id,
						 payment_id)
			return False
...
def push_UpdatePayment_Not_Registred(db, settings, row):
	try:
		new_data = json.loads(row.get('new_data', '{}'))
		new_status = new_data.get('status')
		current_status = new_data.get('currentStatus')
		next_status = new_data.get('nextStatus')
		current_payment_id = new_data.get('payment_id')
		next_payment_id = new_data.get('nextPaymentId')

		ProdCPaymentId = db.fetch_query("""SELECT id_prod FROM payment_session WHERE id = %s AND enabled = 1""",
										(current_payment_id,))
		ProdNPaymentId = db.fetch_query("""SELECT id_prod FROM payment_session WHERE id = %s AND enabled = 1""",
										(next_payment_id,))

		Prod_Current_id = ProdCPaymentId[0]['id_prod']
		Prod_Next_id = ProdNPaymentId[0]['id_prod']
		if not (Prod_Current_id) or not (Prod_Next_id):
			return False
		payload = {
			"currentPaymentId": Prod_Current_id,
			"currentPaymentStatus": current_status,
			"nextPaymentId": Prod_Next_id,
			"nextPaymentStatus": next_status,
			"newStatus": new_status
		}

		success = _send_Update_Payment_Not_Register_Api(settings, payload)
		return success

	except Exception as e:
		print(f"Error: {e} in push_UpdatePamynet_Not_Registed")
		return False
