...
def process_sync_data(db, data, settings):
	...
	if 'formationSubject' in data:
		n = normalize(data['formationSubject'])

		if has_records(n):
			logger.info("Processing FormationSubject")
			from sync.processors.formation_subject_processor import process_formation_subjects
			process_formation_subjects(db, n)
