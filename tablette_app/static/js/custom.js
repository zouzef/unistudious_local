...
async function loadGroups(accountId, sessionId) {
  ...
  try {
    const response = await fetch(
      `/get-group-session/${accountId}/${sessionId}`,
      {
        headers: { "Content-Type": "application/json" },
      },
    );
...
async function loadTeachers(sessionId, groupId) {
  ...
  teacherSelect.innerHTML =
    '<option value="" selected disabled>Select a Subject and Teacher</option>';
  ...
  if (!sessionId || !groupId) {
    _addOption(
      teacherSelect,
      "",
      "Please select a session and group first",
      true,
    );
    ...
    if (response.ok) {
      ...
      if (Array.isArray(teachers) && teachers.length > 0) {
        ...
        _addOption(
          teacherSelect,
          "",
          "No teachers available for this group",
          true,
        );
      }
    } else {
...
function updatePaginationUI() {
  ...
  paginationContainer.style.display =
    totalItems <= itemsPerPage ? "none" : "block";
...
function renderCalendarCards(calendarData) {
  ...
  if (
    ...
    container.innerHTML =
      '<div class="col-12"><p class="text-center">No calendar requests found</p></div>';
    return;
  }
  ...
  container.innerHTML = calendarData.data
    .map((item, index) => {
...
async function initCalendar() {
  const calendareDiv = document.getElementById("calendare");
  const roomId = calendareDiv
    ? parseInt(calendareDiv.dataset.roomId, 10)
    : null;
...
function _buildAutoDescription(formData, groupSelect, teacherSelect) {
  const groupName =
    groupSelect.options[groupSelect.selectedIndex]?.textContent || "Group";
  const teacherText =
...
function _validateFormData(formData) {
  const checks = [
    ...
    [
      !formData.subject_id || !formData.user_id,
  ...
  document
    ...
    ?.addEventListener("click", async function () {
      ...
      const formData = {
        ...
        subject_id:
          teacherSelect.selectedOptions[0]?.getAttribute("data-subject"),
        user_id: teacherSelect.selectedOptions[0]?.getAttribute("data-user"),
        completion_tags: Array.from(
          document.getElementById("eventCompletionTagCalander").selectedOptions,
        ).map((o) => o.value),
        duplicate: document.getElementById("eventDuplicate").value,
        start_date: document.getElementById("eventStartDate").value,
        start_time: getTime("event-start"),
        end_time: getTime("event-end"),
      ...
      if (!formData.description?.trim()) {
        if (
          ...
          formData.description = _buildAutoDescription(
            formData,
            groupSelect,
            teacherSelect,
          );
