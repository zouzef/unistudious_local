...
fetch(`/get-data-account-tablet/${tablet_id}`)
  ...
  .then((result) => {
    if (result.status === "ok" && result.data) {
      document.getElementById("academie-title").textContent =
        result.data.name + " - Calendar";
    }
  })
...
document
  .getElementById("saveAndDisconnectButton")
...
const translations = {
  en: {
    ...
    labelRoom: "Room",
    labelSubject: "Teacher And Subject",
    labelCompletion: "Completion Tag",
    labelDuplicate: "Duplicate",
    labelStartTime: "Start Time",
    labelEndTime: "End Time",
    labelEndDate: "End Date (for recurring events)",
    labelDescription: "Description",
    btnCancel: "Cancel",
    btnSave: "Save",
    btnClose: "Close",
    authTitle: "Teacher Authentication",
    authSubtitle: "Please verify your identity to continue",
    tabPassword: "🔑 Username & Password",
    tabQr: "📷 Scan QR Code",
    labelUsername: "Username",
    labelPassword: "Password",
    btnLogin: "Login & Continue",
  ...
  fr: {
    ...
    labelRoom: "Salle",
    labelSubject: "Enseignant et Matière",
    labelCompletion: "Tag de Complétion",
    labelDuplicate: "Dupliquer",
    labelStartTime: "Heure de début",
    labelEndTime: "Heure de fin",
    labelEndDate: "Date de fin (événements récurrents)",
    labelDescription: "Description",
    btnCancel: "Annuler",
    btnSave: "Enregistrer",
    btnClose: "Fermer",
    authTitle: "Authentification Enseignant",
    authSubtitle: "Veuillez vérifier votre identité pour continuer",
    tabPassword: "🔑 Nom d'utilisateur & Mot de passe",
    tabQr: "📷 Scanner le QR Code",
    labelUsername: "Nom d'utilisateur",
    labelPassword: "Mot de passe",
    btnLogin: "Se connecter & Continuer",
...
function renderStudentList(students) {
  ...
  if (!students.length) {
    container.innerHTML =
      '<p style="text-align:center;color:#aaa;font-size:13px;padding:20px 0;margin:0;">No students found</p>';
    return;
  }
...
function generateDescription() {
  ...
  const dateTimeEnd =
    startDate && endTime ? `${startDate} ${endTime}` : endTime || "";
...
async function loadSpecialGroupDropdowns() {
  ...
  try {
    $(subject2Select).selectpicker("refresh");
  } catch (e) {}
}
...
document.getElementById("specialGroupBackBtn").addEventListener("click", () => {
  bootstrap.Modal.getInstance(
    document.getElementById("specialGroupModal"),
  )?.hide();
  setTimeout(
    () => new bootstrap.Modal(document.getElementById("eventModal")).show(),
    300,
...
document
  .getElementById("specialGroupCloseBtn")
...
document
  ...
  .addEventListener("click", function () {
    ...
    const fields = [
      ...
      {
        id: "sg-capacity",
        errId: "sg-capacity-error",
        msg: "❌ Capacity is required",
        check: (v) => v && parseInt(v) >= 1,
      },
      {
        id: "sg-access-type",
        errId: "sg-access-error",
        msg: "❌ Please select an access type",
        check: (v) => !!v,
      },
      {
        id: "sg-type",
      ...
      {
        id: "sg-session",
        errId: "sg-session-error",
        msg: "❌ Please select a session",
        check: (v) => !!v,
      },
      {
        id: "sg-end-date",
        errId: "sg-startdate-error",
    ...
    const payload = {
      ...
      is_special: true,
      completion_tags: Array.from(
        document.getElementById("sg-completion-tag").selectedOptions,
      ).map((o) => o.value),
