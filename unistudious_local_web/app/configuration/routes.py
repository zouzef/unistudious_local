...
@configuration_bp.route('/api/get_sub_subject/<int:account_subject_id>', methods=['GET'])
def get_sub_subject(account_subject_id):
	try:
		status, response = get_sub_subject_service(account_subject_id)
		return jsonify(response.json()), response.status_code
	except Exception as e:
		return jsonify({
			"Message": f"Error: {e} coming from backend"
